...
Once PASETO authentication is configured, all Graft clients must use a valid token. See the [Graft SQLite config](/sqlite/config) for more information on configuring the token to use with the Graft SQLite extension.
