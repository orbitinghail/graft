...
The Graft Proxy is an optional stateless edge service and caching layer which makes it easier for Graft to replicate to & from devices. Initially it will simply act as a pass-through proxy to object storage with its own authentication and authorization layer.
...
class VolumeWriter:
  ...
  def commit():
    ...
    batch.write(f"log/{vid}/{commit_lsn}", Snapshot {
      vid,
      lsn = commit_lsn,
    ...
    with storage_lock:
      ...
      latest_snapshot = first(f"log/{vid}")
...
The Pull Volume operation only fetches a Volume at a particular LSN.
...
def prepare_commits(local_snapshot, remote_vid, lsn_range):
  snapshots = iter_range(f"log/{handle.remote.vid}/{lsn_range}")
  segments = iter_range(f"segments/{handle.remote.vid}/{lsn_range}")

  # build up an array of commits by mapping each sequential snapshot + associated segments to the next local lsn
