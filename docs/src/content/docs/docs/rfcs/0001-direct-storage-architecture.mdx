...
Each Volume Handle has a name given to it at creation time. The name must match the regex `^[_a-zA-Z][-_a-zA-Z0-9]{0,127}$` (alphanumeric, starts with letter or underscore, max len 128 chars) and be unique on the client.
