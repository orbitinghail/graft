...
#### Local keyspace
Local storage uses Fjall, a partitioned k/v store. In the following keyspace, the top level keys are independent partitions, and the remainder of the keypath is encoded using zerocopy types that preserve lexicographical order.

Values are encoded using zerocopy types, documented in the following section.

```
volumes /
  $vid /
    control -> Control { SyncDirection }
