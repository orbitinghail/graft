...
TODO/Q: do we need to hold the handle lock while updating the local and/or remote volume during push/pull?

##### **`volume_lock(vid)`**

The sharded Volume lock must be held when updating a Volume.

- **Volume commit**: When committing to a Volume, we hold the `volume_lock` to verify the latest snapshot and write out a new snapshot.
- **Checkpoint**: To finish applying a checkpoint, we rewrite the snapshot and segments at the checkpoint LSN. The lock is only held while we update the log, segments, and checkpoints.
...
class VolumeWriter:
  ...
  def commit():
    ...
    with volume_lock(snapshot.vid):
      # verify we are the latest snapshot
      latest_snapshot = last(f"log/{vid}")
...
def push_volume(handle_name):
  (vid, commit) = prepare_push(handle_name)
  remote_commit(vid, commit)

def prepare_push(handle_name):
    ...
    # TODO: continue here
    # basically need to check the local and remote state to compute the sync state
    # then build and push segments
    # then write out the pending commit
    # then commit
    # then take the handle lock to finalize the commit locally
