...
sync / {source_vid} / {source_lsn} -> SyncOp
...
enum SyncStatus {
  ...
  SYNC_STATUS_REJECTED = 3;
}

// SyncOp is stored at `sync/{source_vid}/{source_lsn}`. It is part of our
// two-phase commit protocol used to sync changes between volumes.
//
// The `source_lsn` is the last LSN in the `source_lsns` range. Thus it marks a
// mapping between a LSN on the source Volume and the corresponding LSN on the
// destination volume.
message SyncOp {
  // The Volume we are syncing from.
  bytes source_vid = 1;

  // The LSN range we are attempting to sync from the source volume to the
  // destination volume.
  LsnRange source_lsns = 2;

  // The Volume we are syncing to.
  bytes destination_vid = 3;

  // The resulting LSN in the destination Volume created by this sync operation.
  uint64 destination_lsn = 4;

  // The status of this sync.
  SyncStatus status = 5;

  // The full remote commit we are attempting to sync.
  graft.remote.v1.Commit commit = 5;
}
...
- need better consistent names for writer/upstream throughout
- SyncOps can be created by push and pull - effectively they are mappings
  - may want to split the sync op from the pending commit object
  - can pending commit just land in the destination log directly?
