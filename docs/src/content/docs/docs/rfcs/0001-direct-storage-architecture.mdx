...
segments / {vid} / {lsn} / {min_pageidx, max_pageidx} -> SegmentRef
...
**TODO: open thoughts**
While the current layout is somewhat elegant, its quite difficult to eliminate write amplification when the writer vid fast forwards (or pushes) due to commit collapse during sync.

Its possible to leverage graft's existing local storage impl by adding a indirection layer allowing the same Volume to be opened simultaneously at multiple LSNs.

This will retain some of the remote/local commit mapping complexity, however does solve page write amp (and eliminate fast forwarding) quite well.

Another option is to lean into this much more elegant structure, and fully dedup pages using content hashes.
- benefit: also serves to provide general page dedup (esp useful for sqlite rollback)
- benefit: could be used to add page data hashes to commits
- downside: increase read/write latency due to hashing and indirection
- downside: hashes are 32 bytes which leads to a lot more write amplification, esp since page pointers will be duplicated.

I could give up on full hashing and instead use a lightweight crc + local seqno to build a page id. This value only has to be local. Probably will result in a 16 byte page id (8 byte seqno, 8 byte crc) but be much faster to compute (and takes up half as much space).
