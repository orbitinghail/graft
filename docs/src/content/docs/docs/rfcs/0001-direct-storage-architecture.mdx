...
- **`Volume Handle`**: A reference to a local-remote Volume pair. Also responsible for tracking syncronization between a local and remote Volume. Documented more in the [Volume Handle](#volume-handle) section.
...
To verify data integrity, we attach a hash to each `Commit`. This hash is produced using the following method:

First a sequence of Segment hashes are produced by blake3 hashing the pages in each segment in order by pageidx along with a unique header.
...
Then a Commit hash is produced by blake3 hashing the Segment hashes together with the `page_count` and a unique header.

```python
def commit_hash(page_count, segment_hashes):
  hasher = blake3::new()
  hasher.write(HashHeader::Commit(page_count));
...
Each Volume Handle has a name given to it at creation time. The name must match the regex `/^[a-z0-9-_]+$/` and be unique on the client.
...
message VolumeHandle {
  ...
  // References to the local and remote Volumes, along with LSNs representing their latest successful syncronization.
  graft.remote.v1.VolumeRef local = 2;
  optional graft.remote.v1.VolumeRef remote = 3;

  // Presense of the pending_commit field means that the Push operation is in the process of committing to the remote. If no such Push job is currently running (i.e. it was interrupted), this field must be used to resume or abort the commit process.
...
message PendingCommit {
  ...
  // The associated commit hash. This is used to determine whether or not the commit has landed in the remote, in the case that we are interrupted while attempting to push.
  bytes commit_hash = 2;
}
```
...
does this make gc more tricky? it will need to monitor all open snapshots to determine what's visible. it probably determins vis something like:
...
- somehow store parent checkpoint information directly in the fork's log?
  ...
  - this feels dirty. lets say you start with a fork X on a parent Y
    ...
    - this is particularly bad for mutli-parent forks, where a downstream fork now *must* monitor all of the commits for all upstream parents to track checkpointing
...
4. it may make sense for a volume reader to resolve the checkpoint + parent stack when opening a snapshot. this will reduce the page read cost, and make gc a bit easier. and because we write checkpoints directly to the segment log, those readers will still be able to implictly take advantage of newer checkpoints, although empty pages may still fall through.
...
- currently this requires writing an empty page to a segment
  - except in the case of truncation, in which case the smaller page size implicitely clears all orphaned pages
...
why are pages empty:
1. page is implicitely empty due to being outside of the Volume's page_count
2. the volume was extended by changing the page count, and page does not exist in any visible segment
3. an empty page was explicitely written
...
alternatively we can just make this explicit. truncate + extend results in visibility. thus to explicitely erase a page you would need to either write an empty page, or checkpoint after the truncate.
...
- **GC**: The GC optimistically determines what it can safely to without holding the lock. Once it has a plan of action, GC will incrementally hold this lock while erasing in-accessable snapshots and segments.
