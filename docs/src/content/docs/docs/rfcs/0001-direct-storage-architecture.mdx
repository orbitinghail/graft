...
def pull_volume(vid):
  ...
  # TODO: this needs to recursively pull parents

  # pull changes from object store
  for commit in remote_iter_commits(vid, snapshot.lsn):
...
def remote_iter_commits(vid, start_lsn):
  # return an iterator of Commit's over {prefix}/log/{start_lsn..}
  pass
```
