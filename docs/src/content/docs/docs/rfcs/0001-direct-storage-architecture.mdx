...
message SyncOp {
  ...
  // The full parent commit we are attempting to sync.
...
**SyncOp** thoughts:
- should it be called "Merge"?
- we need SyncOp for these purposes:
  1. to keep track of an in-progress sync, part of our 2PC write protocol
  2. to keep track of the mapping between parent and writer LSNs in order to:
    - know if we have pending local commits to sync
    - to copy cached pages during fast forward
      - this isn't needed if pages are stored by segment id, and segments are shared across Volumes
- so if we don't need to copy over pages, we could reduce this to a single sync state Object
  - writer ref: VolumeRef
    - marks the last successful writer LSN that synced
    - updated when push succeeds and on pull
  - parent ref: VolumeRef
    - marks the parent LSN that maps to the writer ref
    - updated when push succeeds and on pull
  - optional PendingCommit
    - commit: Commit
    - range: LSNRange - the range of LSNs we are trying to sync from the writer
    - set when push starts, cleared when push finishes
