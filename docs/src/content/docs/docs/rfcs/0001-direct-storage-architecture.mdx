...
Build a new Graft runtime which directly interfaces with blob storage, eliminating the need for the MetaStore and PageStore, and setting Graft up as a viable replacement to systems like Litestream. Graft should focus on providing best-in-class PITR, branching, and sparse replication for page-based workloads.
...
Currently Graft requires a MetaStore and PageStore to support replication to and from blob storage. This architecture has the following advantages and disadvantages:
...
In a discussion with a potential user, they expressed reservations due to the layer of indirection between the Graft client and blob storage. Their main argument was that S3 already has proved that it can handle extremely high scale. They would be more comfortable using Graft if clients connected directly to blob storage to pull changes.

### New user experience

Graft should work out of the box without needing additional services to run. By supporting directly interfacing with blob storage or other backends, Graft would be a much more composable client library - making it easier to embed in more places.
...
`graft-rt` implements the Graft runtime which replaces the `graft-client` and `graft-server` crates. The runtime provides access to a remote Volume Catalog, local storage, and client functionality to downstream crates like `graft-sqlite` and `graft-fuse`.

**Volume Catalog**
This subsystem provides push and pull functionality to Volume commit logs. It's designed to support multiple backends, with blob storage being the priority.
...
### New crate: `graft-proxy`

- needed in first pass? or can we just set up for it in the future possibilities section?
- lightweight cache accelerator
- main goal is to allow replicas to more efficiently fast forward to the latest state for a volume, and also support things like volume subscriptions
