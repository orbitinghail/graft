    ...
    while let Some(vref) = cursor.take() {
        let meta = if let Some(meta) = storage.read().volume_meta(vref.vid()).or_into_ctx()? {
            ...
            let checkpoints = remote
                .fetch_checkpoints(vref.vid(), meta.checkpoints().map(|(etag, _)| etag))
                .await
                .or_into_ctx()?;
            if let Some(checkpoints) = checkpoints {
                refresh_checkpoint_commits(storage, remote, checkpoints.1.clone())
                    .await
                    .or_into_ctx()?;
                storage
            ...
            if let Some((_, checkpoints)) = checkpoints.as_ref() {
                refresh_checkpoint_commits(storage, remote, checkpoints.clone())
                    .await
                    .or_into_ctx()?;
            }
            storage
